...
plt.figure(figsize=(8, 6))
plt.scatter(paths_finished['path_length'], paths_finished['durationInSec'], alpha=0.7)
...
slope, intercept, r_value, p_value, std_err = linregress(paths_finished['path_length'], paths_finished['durationInSec'])
x = np.array(paths_finished['path_length'])
y = slope * x + intercept

# Plot the linear regression line
plt.plot(x, y, color='red', label=f'Linear Regression (R-squared = {r_value**2:.2f})')
plt.legend()
...
flat_visited_articles = [article for path in paths_finished['path'] for article in path]
...
paths_finished['uk_percentage_position'] = paths_finished['path'].apply(calculate_percentage_position)
...
cat_all = categories_fct(['category1', 'category2', 'category3'])
print('All categories:',cat_all)
cat_1 = categories_fct(['category1'])
print('Category 1:', cat_1)
subcat2 = categories_fct(['category2'])
print('Category 2', subcat2)
subcat3 = categories_fct(['category3'])
...
def analyze_articles_near(df, reference_article, steps_away, namecat):
    results = []
    n_results = []
    ...
    for _, row in df.iterrows():
        ...
        if reference_article in path:
            ...
            articles_to_analyze = [article for article in articles_to_analyze if article != reference_article]
            
            # Collect the categories of the articles to analyze
            categories_of_articles_to_analyze = []
            for article in articles_to_analyze:
                article_categories = categories.loc[categories['article'] == article, namecat]
                article_categories = article_categories.values.flatten()
                article_categories = [cat for cat in article_categories if pd.notna(cat)]
...
results_1_step, non1 = analyze_articles_near(paths_finished, 'United_Kingdom', 1, ['category2'])
results_2_steps, non2 = analyze_articles_near(paths_finished, 'United_Kingdom', 2, ['category2'])
results_3_steps, non3 = analyze_articles_near(paths_finished, 'United_Kingdom', 3, ['category2'])
...
categories_art = categories.loc[categories['article'] == 'William_Shakespeare', ['category1', 'category2', 'category3']]
...
article_categories = categories_art
...
# results that coincide with the categories of the "UK" article
all_results = results_1_step + results_2_steps + results_3_steps
# results that do not are denoted by "non"
...
combined_counts_1 = {cat: category_counts_1.get(cat, 0) for cat in set(category_counts_1)}
combined_ncounts_1 = {cat: category_ncounts_all.get(cat, 0) for cat in set(category_ncounts_all)}

combined_counts_2 = {cat: category_counts_2.get(cat, 0) for cat in set(category_counts_2)}
combined_ncounts_2 = {cat: category_ncounts_all.get(cat, 0) for cat in set(category_ncounts_all)}

combined_counts_3 = {cat: category_counts_3.get(cat, 0) for cat in set(category_counts_3)}
combined_ncounts_3 = {cat: category_ncounts_all.get(cat, 0) for cat in set(category_ncounts_all)}

combined_counts_all = {cat: category_counts_all.get(cat, 0) for cat in set(category_counts_all)}
combined_ncounts_all = {cat: category_ncounts_all.get(cat, 0) for cat in set(category_ncounts_all)}

# Create a list of colors where highlighted categories are in a different color
colors = ['skyblue' if cat not in article_categories else 'gold' for cat in categories_list_all]

# Create a bar plot for Step 1
categories_list_1 = sorted(list(set(combined_counts_1.keys()) | set(combined_ncounts_1.keys())))
...
plt.bar(index_1 + bar_width, ncounts_1, color='lightcoral', label='Do not coincide with UK')
plt.xlabel('Category')
plt.ylabel('Count')
...
for category in article_categories:
    if category in categories_list_all:
        ...
        if category_index > 0 and arrow_position - 0.1 < bar_positions[category_index - 1]:
            arrow_position = bar_positions[category_index - 1] + 0.1
        elif category_index < len(categories_list_all) - 1 and arrow_position + 0.1 > bar_positions[category_index + 1]:
            arrow_position = bar_positions[category_index + 1] - 0.1
        
        plt.annotate('^', xy=(arrow_position, max(counts_all[category_index], ncounts_all[category_index]) + 50),
...
categories_list_2 = sorted(list(set(combined_counts_2.keys()) | set(combined_ncounts_2.keys())))
...
plt.bar(index_2 + bar_width, ncounts_2, color='lightcoral', label='Do not coincide with UK')
plt.xlabel('Category')
plt.ylabel('Count')
...
categories_list_3 = sorted(list(set(combined_counts_3.keys()) | set(combined_ncounts_3.keys())))
...
plt.bar(index_3 + bar_width, ncounts_3, color='lightcoral', label='Do not coincide with UK')
plt.xlabel('Category')
plt.ylabel('Count')
...
categories_list_all = sorted(list(set(combined_counts_all.keys()) | set(combined_ncounts_all.keys())))
...
plt.bar(index_all + bar_width, ncounts_all, color='lightcoral', label='Do not coincide with UK')
plt.xlabel('Category')
plt.ylabel('Count')
...
ax = sns.catplot(x='rating', y='length_path', data=paths_finished, kind="bar", palette="coolwarm")
